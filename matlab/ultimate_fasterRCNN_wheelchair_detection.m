...
%  On this code I test Faster-RCNN on a wheelchair dataset using different
%  backbone architectures: VGG16, VGG19, AlexNet and also give option to
%  create and test with own networks. The objective of this approach is to 
%  detect wheelchairs given a cluttered environment and analyze how well 
%  each model is able to predict the prescence of a wheelchair.  

%  This implementation borrows code from this tutorial:
...
%  Some functions used in this code are found in the 'learning_functions'
%  folder. 

%  Some other important details: 
%   - Defined RoIs using Training Image Labeler. 
%   - Total dataset size: 800, 445, 370 images containing only wheelchairs. 
%   - Tested on a GeForce GT 750M, 4Gb, 386 cores. 
%   - Image sizes tested: 320, 256, 227 
...
% layers from the vgg16 model. This will help reducing the computational 
% cost of training from scratch. 
net = 'navnet';

if strcmp(net, 'vgg16')
    backbone = vgg16;
elseif strcmp(net, 'vgg19')
    backbone = vgg19;
elseif strcmp(net, 'alexnet')
    backbone = alexnet;
else 
    backbone = load(fullfile(matlabroot,'toolbox','vision','visiondata', ... 
...
% When testing with data augmentation: 
...
input_layer = imageInputLayer([32 32 3], ...      % size
                 ...
                 'DataAugmentation', 'none', ...  % data augmentation
